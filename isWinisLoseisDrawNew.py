def isWin( gameState , x , y , color):
    ...
    while ( x - k ) >= 0 and ( y - k ) >= 0:
        if gameState["Board"][x - k][y - k] != color:
            break
        k += 1
...
def isDraw( gameState , x , y , color ):
    # 参数当前状态gameState
    # 参数当前状态x , y为新棋子的位置，color为新棋子颜色
    # 返回bool类型，是否平局
    # 检查是否有棋子未下
    for j in range(7) :
        if gameState["Board"][0][j]== -1:
            return False
    
    if isWin(gameState , x , y , color) == True  :
        return False
